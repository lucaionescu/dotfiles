...
function setup_environment {
  bf=""
  n=""
  ORGANISATION="Leibniz-Rechenzentrum (LRZ) der Bayerischen Akademie der Wissenschaften"
  URL="https://servicedesk.lrz.de"
  SUPPORT="servicedesk@lrz.de"
if [ ! -z "$DISPLAY" ] ; then
...
if ! ask "Dieses Installationsprogramm funktioniert nur für Anwender von ${bf}Leibniz-Rechenzentrum (LRZ) der Bayerischen Akademie der Wissenschaften.${n}" "Weiter" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:radius.lrz.de'})
        server_name = 'radius.lrz.de'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
        ...
        s_8021x = dbus.Dictionary({
            ...
            'anonymous-identity': 'anonymous@eduroam.mwn.de',
        })
